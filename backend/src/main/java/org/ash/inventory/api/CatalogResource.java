...
public class CatalogResource {
    ...
    public List<?> items(@QueryParam("search") String search) {
        actor.current();
        List<Item> values = search == null || search.isBlank()
                ? Item.list("active = true order by createdAt desc")
                : Item.list("active = true and lower(name) like ?1 order by name", "%" + search.toLowerCase() + "%");
    ...
    public List<?> locations() { actor.current(); return StorageLocation.<StorageLocation>list("order by createdAt desc").stream().map(mapper::location).toList(); }
    ...
    public List<?> assemblies() { actor.current(); return Assembly.<Assembly>list("order by createdAt desc").stream().map(mapper::assembly).toList(); }
    ...
    public List<?> events(@QueryParam("eventType") String eventType) {
        actor.current();
        List<EventOccurrence> values = eventType == null ? EventOccurrence.list("order by startDate desc") : EventOccurrence.list("eventType = ?1 order by startDate desc", eventType);
    ...
    public List<?> factions(@QueryParam("eventType") String eventType) {
        actor.current();
        List<Faction> values = eventType == null ? Faction.list("active = true order by eventType, name") : Faction.list("active = true and eventType = ?1 order by name", eventType);
