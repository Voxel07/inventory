...
export function TransactionHistoryPage() {
  ...
  const { pageItems: pageTransactions, page: currentPage, setPage, pageSize, onPageSizeChange } = useClientPagination(filteredTransactions);

  function updateFilters(patch: Partial<typeof transactionFilters>) {
  ...
  return (
    <Box>
      ...
      <Stack direction="row" spacing={2} sx={{ mb: 3, flexWrap: 'wrap' }} useFlexGap>
        ...
        <TextField select label={t('Artikel', 'Item')} value={transactionFilters.itemId} onChange={(event) => updateFilters({ itemId: event.target.value })} size="small" sx={{ minWidth: 150 }}>
          <MenuItem value="">{t('Alle Artikel', 'All items')}</MenuItem>
          {items?.map((item) => <MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}
        </TextField>
        <TextField select label={t('Benutzer', 'User')} value={transactionFilters.userId} onChange={(event) => updateFilters({ userId: event.target.value })} size="small" sx={{ minWidth: 150 }}>
        ...
        <TextField select label={t('Typ', 'Type')} value={transactionFilters.transactionType} onChange={(event) => updateFilters({ transactionType: event.target.value })} size="small" sx={{ minWidth: 120 }}>
      ...
      <ListPagination pageSize={pageSize} onPageSizeChange={onPageSizeChange} count={filteredTransactions.length} page={currentPage} onChange={setPage} loadingMore={!isError && (hasNextPage || isFetchingNextPage)} loadError={isError} onRetry={() => { void refetch(); }} />
    </Box>
  );
}
